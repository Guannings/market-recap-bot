...
def build_html(data, asof_label):
...
<h2 style="margin:0 0 2px">Market Recap</h2>
<div style="color:#666;font-size:13px;margin-bottom:18px">{asof_label}</div>
<h3 style="{h3}">Market News — what moved things</h3>{headlines_html(data['news'])}
<h3 style="{h3}">US Indices (close)</h3>{index_table(data['us'])}
<h3 style="{h3}">Europe (close)</h3>{index_table(data['eu'])}
<h3 style="{h3}">FX</h3>{index_table(data['fx'])}
<h3 style="{h3}">Rates</h3>{rates_table(data['rates'])}
<h3 style="{h3}">Commodities</h3>{index_table(data['commodities'])}
<div style="font-size:12px;color:#888;border-top:1px solid #eee;padding-top:10px">
Auto-generated by market_recap.py. Market data: Yahoo Finance. Headlines: reputable finance RSS
(China outlets, Musk-firm services, and content farms excluded). Sanity-check figures before acting.
</div></div>"""


def build_text(data, asof_label):
    lines = [f"MARKET RECAP — {asof_label}", ""]
    ...
    def block(title, rows, rate=False):
    ...
    lines += ["", "Data: Yahoo Finance + reputable finance RSS. Sanity-check before acting."]
    return "\n".join(lines)
...
def main():
    ...
    # Determine the "as of" date from US data when available.
    ...
    html = build_html(data, asof_label)
    text = build_text(data, asof_label)
